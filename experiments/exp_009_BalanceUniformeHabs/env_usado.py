...
class RobotEnv(gym.Env):
    ...
    def step(self, action):
        ...
        # --- Calcular recompensa (densa por paso) ---

        if final_state['depleted']:
            reward     = -10.0
            terminated = True
        else:
            # Visita a una habitacion
            if target_node in ROOM_NODES:
                self._visit_counts[target_node] += 1

                visits = np.array([self._visit_counts[n] for n in ROOM_NODES], dtype=np.float32)
                total_rooms = float(np.sum(visits))

                if total_rooms <= 0.0:
                    reward = 0.0
                else:
                    p = visits / total_rooms
                    desbalanceo = float(np.sum((1.0 / float(len(ROOM_NODES)) - p) ** 2))
                    max_desbalanceo = (float(len(ROOM_NODES)) - 1.0) / float(len(ROOM_NODES))
                    normalized = (max_desbalanceo - desbalanceo) / max_desbalanceo if max_desbalanceo > 0.0 else 0.0
                    reward = float(10.0 * np.clip(normalized, 0.0, 1.0))

            # Visita a C -> penalización proporcional a su frecuencia
            elif target_node == 'C':
                self._visit_counts['C'] += 1
                total_all = float(sum(self._visit_counts[n] for n in ALL_NODES))
                if total_all <= 0.0:
                    c_freq = 0.0
                else:
                    c_freq = float(self._visit_counts['C']) / total_all
                reward = -0.2 * c_freq

        # Actualizar conteo de pasos y truncado por longitud de episodio.
